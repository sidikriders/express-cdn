  ...
  return function(err, results) {
    ...
    switch (method) {
      case 'uglify':
        ...
        var babelResult = babel.transform(results, {
          presets: [
            'env',
            'minify'
          ]
        })
        if (babelResult.error !== undefined) {
          logger({
            task: 'express-cdn',
            message: 'Failed to minify ' + fileName + ' due to babel error.'});
          throwError(babelResult.error);
        } else {
          pushFileToS3(S3, babelResult.code, fileName, headers, callback);
        }
        break;
      case 'minify':
        if (!(results instanceof Array)) {
